...
def test_simple(encoder, depth_decoder, input_image, original_width, original_height, pred_metric_depth, idx, output_directory, saving_flag=True):
    ...
    with torch.no_grad():
        ...
        # Saving numpy file
        output_name = "checkimage"#os.path.splitext(os.path.basename(image_path))[0]
        scaled_disp, depth = disp_to_depth(disp, 0.1, 100)
        depth_resized = torch.nn.functional.interpolate(depth, (original_height, original_width), mode="bilinear", align_corners=False)
        if saving_flag:
...
#def test_simple(encoder, depth_decoder, feed_height, feed_width, input_image, pred_metric_depth, idx, saving_flag=True):
        test_simple(encoder=encoder, depth_decoder=depth_decoder, input_image=input_image, pred_metric_depth=args.pred_metric_depth, idx=idx, output_directory=output_directory, saving_flag=True)
